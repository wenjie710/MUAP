...
def get_accuracy(train_set_loader, resnet50, densenet121, vgg16):
    ...
    resnet50_acc = np.array(acc_duke_model).mean()
    densenet121_acc = np.array(acc_duke_model).mean()
    vgg16_acc = np.array(acc_duke_model).mean()
...
def main():
    ...
    result.append(mAP)
    print('mAP ', mAP, 'drop rate ', (0.759-mAP)/0.759)
    ...
    cmc, mAP = get_map(duke_model, duke_num_query, val_query_loader, val_gal_loader)
    result.append(mAP)
    print('mAP ', mAP, 'drop rate ', (0.734-mAP)/0.734)
    ...
    cmc, mAP = get_map(duke_model, duke_num_query, val_query_loader, val_gal_loader)
    result.append(mAP)
    print('mAP ', mAP, 'drop rate ', (0.663-mAP)/0.663)
    ...
    cmc, mAP = get_map(duke_model, duke_num_query, val_query_loader, val_gal_loader)
    result.append(mAP)
    print('mAP ', mAP, 'drop rate ', (0.669-mAP)/0.669)
    ...
    new_name = os.path.basename(attack_path).split('.')[0]+'_botdukemdr.npy' #_medianfilter.npy'
